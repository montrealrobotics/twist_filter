...
class IIRFilterNode(Node):
    def __init__(self):
        super().__init__('iir_twist_filter')
        ...
        config = {
            ...
            'num_out_samples': self.get_parameter('num_out_samples').value,
            'out_weights': self.get_parameter('out_weights').value,
        }
        
        self.get_logger().info(f'Active filters: {active_filters}')

        self.component_filters = IIRTwistFilterObject(active_filters, config)
